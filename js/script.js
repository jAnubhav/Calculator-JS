...
const addText = text => newText(entry.innerHTML + text);
...
const createDiv = (className, id, ...child) => {
    ...
    div.append(...child); return div;
}

const createButton = (text, className, func = addInEntry) => createNode("button", `d-grid btn round ${className}`, {
    innerHTML: text, onclick: () => func(text) });
...
[[Array.from("789456123.0"), ""], [Array.from("/*-+"), "operators"], [["DEL", "AC"], "clearings"]].forEach(
    ([keys, className], index) => keys.forEach(e => containers[index].append(createButton(e, className))));
